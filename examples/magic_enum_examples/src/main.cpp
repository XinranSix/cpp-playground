...
int main() {
    ...
    if (c2.has_value()) {
        std::cout << "BLUE = " << to_integer(c2.value())
                  << std::endl; // BLUE = 0
    }
    ...
    using magic_enum::iostream_operators::operator<<; // out-of-the-box ostream
                                                      // operator for all enums.
    ...
    std::cout << "Color[0] = " << magic_enum::enum_value<Color>(0)
              << std::endl; // Color[0] = RED
